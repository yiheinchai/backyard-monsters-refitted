...
export class HOUSINGBUNKER extends Bunker {
    ...
    override Destroyed(param1: boolean = true): void {
        ...
        const _loc2_: boolean = MapRoomManager.instance.isInMapRoom3;
        let _loc3_: number = 0;
    ...
    public FindTargets(param1: number, param2: number = 1): void {
        ...
        let _loc3_: any[] = Targeting.getCreepsInRange(GLOBAL._buildingProps[127].stats[this._lvl.Get() - 1].range, this._position.add(new Point(0, this._footprint[0].height / 2)), Targeting.getOldStyleTargets(0));
        this._targetCreeps = this.addTargetCreeps(param1, _loc3_, param2);
        if (this.canTargetAir()) {
            _loc3_ = Targeting.getCreepsInRange(GLOBAL._buildingProps[127].stats[this._lvl.Get() - 1].range, this._position.add(new Point(0, this._footprint[0].height / 2)), Targeting.getOldStyleTargets(2));
