...
export class MARKETING {
        ...
        try {
            switch (key) {
                ...
                case "planner":
                    break;
                case "hcc":
                    const hatCount = InstanceManager.getInstancesByClass(BUILDING13).length;
                    ...
                    break;
                case "storagemore":
                    const siloCount = InstanceManager.getInstancesByClass(BUILDING6).length;
                    let quantityIndex = GLOBAL.townHall._lvl.Get() - 1 < GLOBAL._buildingProps[5].quantity.length ? GLOBAL.townHall._lvl.Get() - 1 : GLOBAL._buildingProps[5].quantity.length - 1;
                    let canbuild = siloCount < GLOBAL._buildingProps[5].quantity[quantityIndex];
                    const tmpr1 = BASE._resources.r1.Get() >= 0.8 * BASE._resources.r1max;
                    const tmpr2 = BASE._resources.r2.Get() >= 0.8 * BASE._resources.r2max;
                    const tmpr3 = BASE._resources.r3.Get() >= 0.8 * BASE._resources.r3max;
                    const tmpr4 = BASE._resources.r4.Get() >= 0.8 * BASE._resources.r4max;
                ...
                case "storageupgrade":
                    let sc = 0;
                    const storageInstances = InstanceManager.getInstancesByClass(BUILDING6);
                    ...
                    quantityIndex = GLOBAL.townHall._lvl.Get() - 1 < GLOBAL._buildingProps[5].quantity.length ? GLOBAL.townHall._lvl.Get() - 1 : GLOBAL._buildingProps[5].quantity.length - 1;
                    canbuild = sc < GLOBAL._buildingProps[5].quantity[quantityIndex];
                    const tmpr1b = BASE._resources.r1.Get() >= 0.8 * BASE._resources.r1max;
                    const tmpr2b = BASE._resources.r2.Get() >= 0.8 * BASE._resources.r2max;
                    const tmpr3b = BASE._resources.r3.Get() >= 0.8 * BASE._resources.r3max;
                    const tmpr4b = BASE._resources.r4.Get() >= 0.8 * BASE._resources.r4max;
                    if (!canbuild && tgtb && (tmpr1b || tmpr2b || tmpr3b || tmpr4b) && GLOBAL.Timestamp() - GLOBAL.StatGet("CM10") > 60 * 60 * 24 * 5) {
                ...
                case "mushroompick":
                    break;
                case "laser":
                    const laser = InstanceManager.getInstancesByClass(BUILDING23).length > 0;
                    ...
                    break;
                case "tesla":
                    const tesla = InstanceManager.getInstancesByClass(BUILDING25).length > 0;
            ...
            LOGGER.Log("err", "MARKETING.Show " + key + " " + e.stack);
        }
        return found;
    }
    ...
    public static Process(): void {
        ...
        try {
            ...
            LOGGER.Log("err", "MARKETING.Process " + e.stack);
        }
    }
}
