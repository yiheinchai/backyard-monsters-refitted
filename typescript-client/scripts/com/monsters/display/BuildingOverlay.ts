...
export class BuildingOverlay {
    ...
    public static Setup(building: BFOUNDATION): void {
        ...
        if (!BuildingOverlay._isSetup) {
            ...
            BuildingOverlay.u_bmd = ImageText.Get(getKEYS().Get("bdg_state_upgrading"), 9, 0.6, filters);
            BuildingOverlay.r_bmd = ImageText.Get(getKEYS().Get("bdg_state_repairing"), 9, 0.6, filters);
            BuildingOverlay.b_bmd = ImageText.Get(getKEYS().Get("bdg_state_building"), 9, 0.6, filters);
            BuildingOverlay.f_bmd = ImageText.Get(getKEYS().Get("bdg_state_fortifying"), 9, 0.6, filters);
        ...
        if (!BYMConfig.instance.RENDERER_ON) {
            ...
            getMAP()._BUILDINGTOPS.addChild(container);
        }
        
        if (getGLOBAL()._aiDesignMode && BuildingOverlay.k_SHOW_DEBUG_HEALTH) {
            ...
            if (building._repairing) {
                ...
                if (building._prefab) {
                    for (let i = 0; i < building._prefab; i++) {
                        totalTime += getGLOBAL()._buildingProps[building._type - 1].costs[i].time.Get();
                    }
                } else {
    ...
    protected static clearOverlay(data: BuildingOverlayData): void {
        if (data.container && data.container.parent === getMAP()._BUILDINGTOPS) {
            getMAP()._BUILDINGTOPS.removeChild(data.container);
        }
