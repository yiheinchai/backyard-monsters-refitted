/**
 * Event store prize data - static data for all available event store prizes.
 */
export class EventStorePrizeData {
    private static readonly AVAILABLE_ITEMS: any[] = [
        {
            id: "prize_rezghul",
    ...
    /**
     * Find event store prize data by ID.
     * @internal
     */
    public static FindEventStorePrizeData(prizeId: string): any | null {
        const length = EventStorePrizeData.AVAILABLE_ITEMS.length;
        for (let i = 0; i < length; i++) {
            const item = EventStorePrizeData.AVAILABLE_ITEMS[i];
            if (item.id === prizeId) {
                return item;
            }
        }
