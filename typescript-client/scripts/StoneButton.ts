...
/**
 * StoneButton - Stone-styled button class
 * Handles stone/lava button rendering with text
...
export class StoneButton extends Sprite {
    ...
    public Setup(param1: string, param2: number = 12): void {
        let tgt: number;
        let tgtDiff: number;
        let imgArray: any[];
        let i: number;
        ...
        const sz: number = param2;

        const cbf = (param1: string, param2: BitmapData): void => {
            const _loc3_: BitmapData = new BitmapData(param2.width, param2.height, true, 0);
            _loc3_.copyPixels(param2, new Rectangle(0, 0, param2.width, param2.height), new Point(0, 0));
            const _loc4_: number = this._tgtWidth / _loc3_.width;
            const _loc5_: ScaleBitmap = new ScaleBitmap(_loc3_);
            ...
            const _loc6_: Matrix = new Matrix();
