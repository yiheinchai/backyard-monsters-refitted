...
export class PlayerLayer extends Sprite {
    ...
    public mapWidth: number = 1500;
    public player: PlayerBase | null = null;
    public _wmbToDisplay: number = 3;
    ...
    public Tick(...args: any[]): void {
        ...
        if (this._frameNumber % 40 === 0) {
            ...
            if (MapRoom.BRIDGE.GLOBAL._flags.attacking === 0) {
                msg = String(MapRoom.BRIDGE.KEYS.Get("map_msg_attackingdisabled"));
            }
            if (!msg) {
            }
        }
    ...
    public Get(): void {
        const handleLoadSuccessful = (obj: Record<string, any>): void => {
            try {
                MapRoom.BRIDGE.GLOBAL.WaitHide();
                if (obj.error === 0) {
                    obj.wmbases = [];
                    const aib = MapRoom.BRIDGE.WMBASE._bases;
                    try {
                        if (aib) {
                            for (const ai in aib) {
                                if (aib[ai]) {
    ...
    public Create(data: Record<string, any>): void {
        ...
        if (Boolean(data) && Boolean(data.wmbases)) {
            for (let i = 0; i < data.wmbases.length; i++) {
                if (this.wmBasesUsed < this._wmbToDisplay) {
                    const rawData = data.wmbases[i];
                    baseExists = false;
        ...
        if (this.basesForeign.length < this._playersLimit && data && Boolean(data.bases)) {
            ...
            if (this.basesForeign.length + data.bases.length >= this._playersLimit) {
                ...
                limit = data.bases.length;
            }
            if (data && data.bases && data.bases.length > 0) {
                for (let i = 0; i < limit; i++) {
                    const rawData = data.bases[i];
                    baseExists = false;
                    for (const existingData of this.baseData) {
                        if (existingData.baseid.Get() === rawData.baseid) {
                            existingData.Update(rawData);
                            existingData.online = rawData.saved >= MapRoom.BRIDGE.GLOBAL.Timestamp() - 62;
                            baseExists = true;
                            break;
                        }
                    }
    ...
    public setMapCoords(base: any, isWM: boolean = false): boolean {
        ...
        let isValid = true;
        if (isWM) {
            const charMap: Record<string, number> = { "l": 0, "k": 1, "a": 2, "d": 3 };
            const charIndex = charMap[base.data.basename.charAt(0).toLowerCase()];
            mapX = Obstruction.Reserved[charIndex].x / this.divisor;
            mapY = (Obstruction.Reserved[charIndex].y - 130) / this.divisor;
            ...
            const gridSize = this.mapWidth / this.divisor - 2;
            mapX = 1 + base.data.baseid.Get() % gridSize;
            mapY = 1 + base.data.baseseed.Get() % gridSize;
            const coords = this.getNonConflictingCoords(new Point(mapX, mapY), new Rectangle(2, 2, gridSize - 2, gridSize - 2), 10);
            if (!coords) {
                return false;
            }
            base.mapX = coords.x;
        ...
        base.x = 130 + this.divisor * base.mapX + jitterValue;
        base.y = 130 + this.divisor * base.mapY + jitterValue;
        return isValid;
    }
