...
export class PlayerHandler extends Sprite {
    ...
    public configure(player: any): Record<string, any> {
        ...
        if (this.data!.wm.Get() === 0) {
            ...
            if (this.data!.friend.Get() === 1) {
                relation = getKEYS().Get("map_status_friends");
                relationColor = "#0000FF";
            }
            if (this.data!.attacksfrom.Get() > 1) {
                relation = getKEYS().Get("map_status_hostile");
            ...
            if (Boolean(this.data!.trucestate) && this.data!.trucestate !== "") {
                ...
                if (this.data!.trucestate === "accepted") {
                    okAttack = false;
                    relation = getKEYS().Get("map_status_tactive");
                    relationColor = "#00FF00";
                } else if (this.data!.trucestate === "requested") {
                    relation = getKEYS().Get("map_status_trequested");
                    relationColor = "#0000FF";
                } else if (this.data!.trucestate === "rejected") {
                    relation = getKEYS().Get("map_status_trejected");
                    relationColor = "#CC0000";
                }
                switch (this.data!.attackpermitted.Get()) {
                    case 5:
                        okAttack = false;
                        extraStatus = getKEYS().Get("map_status_dp");
                        break;
                    case 6:
                        okAttack = false;
                        extraStatus = getKEYS().Get("map_status_sp");
                        break;
                    case 7:
                        okAttack = false;
                        okView = false;
                        extraStatus = getKEYS().Get("map_status_underattack");
                ...
                switch (this.data!.attackpermitted.Get()) {
                    case 3:
                        okAttack = false;
                        extraStatus = getKEYS().Get("map_status_level");
                        break;
                    case 4:
                        extraStatus = getKEYS().Get("map_status_vengeance", { "v1": this.data!.retaliatecount });
                        extraStatusColor = "#FF0000";
                        break;
                    case 5:
                        okAttack = false;
            ...
            if (this.data!.truceexpire > 0) {
                extraStatus = getKEYS().Get("map_status_timeremain", { "v1": getGLOBAL().ToTime(this.data!.truceexpire, true, false) });
    ...
    private onMessage(event: MouseEvent): void {
        ...
        getGLOBAL()._layerWindows.addChild(message);
    }
    ...
    private onTruce(event: MouseEvent): void {
        getSOUNDS().Play("click1");
        getGLOBAL().Message(getKEYS().Get("msg_infernotruce"));
    }
    ...
    private onAttack(event: MouseEvent): void {
        ...
        let okAttack = false;
        let message = "";
        const attackLabel = getKEYS().Get("map_attack_btn2");
        const hasMonsters = Boolean(this._BRIDGE.HOUSING) && this._BRIDGE.HOUSING._housingUsed.Get() > 0 || getGLOBAL()._playerGuardianData !== null && !MAPROOM_DESCENT.DescentPassed;
        if (baseData.wm.Get() === 0) {
            if (baseData.saved.Get() >= getGLOBAL().Timestamp() - 62) {
                okAttack = false;
                message = getKEYS().Get("map_msg_ownerinyard", { "v1": baseData.ownerName });
            } else if (getGLOBAL()._flags.attacking === 0) {
                okAttack = false;
                message = getKEYS().Get("map_msg_attackingdisabled");
            } else {
                switch (baseData.attackpermitted.Get()) {
                    case 1:
                        okAttack = true;
                        if (getBASE()._isProtected > getGLOBAL().Timestamp()) {
                            message = getKEYS().Get("map_msg_protection", { "v1": getGLOBAL().ToTime(getBASE()._isProtected - getGLOBAL().Timestamp(), false, false) });
                        } else if (baseData.friend.Get()) {
                            message = getKEYS().Get("map_msg_attackfriend", { "v1": baseData.ownerName });
                        } else {
                            message = getKEYS().Get("map_msg_attackconfirm", { "v1": baseData.ownerName });
                        }
                        break;
                    case 2:
                        okAttack = true;
                        message = getKEYS().Get("map_msg_higherlevelconfirm", { "v1": baseData.ownerName });
                        break;
                    case 3:
                        message = getKEYS().Get("map_msg_leveltoolow");
                        break;
                    case 4:
                        okAttack = true;
                        message = getKEYS().Get("map_msg_vengeance");
                        break;
                    case 5:
                        message = getKEYS().Get("map_msg_dp", { "v1": baseData.ownerName });
                        break;
                    case 6:
                        message = getKEYS().Get("map_msg_sp", { "v1": baseData.ownerName });
                        break;
                    case 7:
                        message = getKEYS().Get("map_msg_inprogress", { "v1": baseData.ownerName, "v2": baseData.attacker });
                        break;
                    case 9:
                        message = getKEYS().Get("map_msg_truceactive", { "v1": baseData.ownerName });
                        break;
                }
            }
        } else {
            okAttack = true;
            if (baseData.wm.Get() === 0) {
                message = getKEYS().Get("map_msg_atatckconfirm", { "v1": baseData.ownerName });
                if (baseData.level.Get() > getBASE().BaseLevel().level) {
                    message = getKEYS().Get("map_msg_higherlevelconfirm", { "v1": baseData.ownerName });
                }
            } else {
        ...
        if (okAttack) {
            ...
            const mode = baseData.wm.Get() === 1 ? "iwmattack" : "iattack";
            if (hasMonsters) {
                getGLOBAL().Message(message, attackLabel, this.onAttackB.bind(this), [Number(baseData.baseid.Get()), mode]);
            } else {
                if (MAPROOM_DESCENT.DescentPassed) {
                    message = getKEYS().Get("infmap_msg_nomonsters");
                } else {
                    message = getKEYS().Get("map_msg_nomonsters");
                }
                getGLOBAL().Message(message);
            }
        } else {
            getGLOBAL().Message(message);
        }
    ...
    public onAttackB(baseId: number, mode: string): void {
        ...
        getBASE().LoadBase(null, 0, Number(baseId), mode, false, EnumYardType.INFERNO_YARD);
        if (!MAPROOM_DESCENT.DescentPassed && mode === "iwmattack") {
            getLOGGER().Stat([87, baseData.level.Get(), "Attacked"]);
        }
