...
export class LOGGER {
    ...
    public static Log(logType: string, message: string, force: boolean = false): void {
        ...
        if (force || !getGLOBAL()._flags || (getGLOBAL()._flags && getGLOBAL()._flags.gamedebug === 1)) {
            if (!LOGGER._logged[logType + message]) {
                if (logType === "hak" || logType === "err" || logType === "log") {
                    LOGGER._logged[logType + message] = 1;
                }
                message = "" + "[v" + getGLOBAL()._version.Get() + "" + "r" + getGLOBAL()._softversion + "] " + message + " [mode: " + getGLOBAL()._loadmode + " baseid:" + getBASE()._baseID + "]";
                const data: [string, string | number][] = [["key", logType], ["value", message], ["saveid", getBASE()._lastSaveID]];
                console.log(data.toString());
                new (getURLLoaderApi())().load(getGLOBAL()._apiURL + "player/recorddebugdata", data, LOGGER.handleLoadSuccessful, LOGGER.handleLoadError);
            }
        }
    }
    ...
    public static Stat(data: any[]): void {
        if (!getGLOBAL()._flags.gamestatsb) {
            return;
        }
        try {
            ...
            if (data[0] >= 1 && data[0] <= 4 || data[0] === 26 || data[0] === 51) {
                ...
                if (data[0] === 1) st2 = getGLOBAL().e_BASE_MODE.BUILD;
                ...
                name = "level" + data[3];
                st3 = String(getCHAMPIONCAGE()._guardians[data[1]].name);
                ...
                st1 = getBASE().isInfernoMainYardOrOutpost ? "storeinf" : "store";
                st2 = String(data[1]);
                ...
                st3 = String(getCHAMPIONCAGE()._guardians["G" + data[1]].name);
                ...
                st1 = "siege-weapon";
                const siegeNames: Record<number, string> = {90: "unlock", 91: "upgrade", 92: getGLOBAL().e_BASE_MODE.BUILD, 93: "used_in_battle", 94: "vacuumloot"};
            ...
            if (st1) {
                ...
                if (getBASE().isOutpost) o.st1 = "outpost-" + o.st1;
                if (getBASE().isMainYardInfernoOnly) o.st1 = "inferno-" + o.st1;
    ...
    public static StatB(stats: Record<string, any>, name: any): void {
        if (!getGLOBAL()._flags || !getGLOBAL()._flags.gamestatsb) {
            return;
        }
        stats.level = getBASE().BaseLevel().level;
        getGLOBAL().CallJS("cc.recordEvent", [name, stats], false);
    }
    ...
    public static KongStat(data: any[]): void {
        ...
        try {
            ...
            if (arg) {
                getGLOBAL().CallJS("cc.kg_statsUpdate", [arg], false);
            }
        } catch (e: any) {
