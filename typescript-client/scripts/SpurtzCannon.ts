...
/**
 * SpurtzCannon - Spurtz Cannon tower class
 * Fires spurtz projectiles that can spawn Spurtz creatures
...
export class SpurtzCannon extends BTOWER {
    ...
    private _targetCreepIndex: number = 0;
    ...
    public override TickFast(param1: Event | null = null): void {
        super.TickFast(param1);
        this.killSpurts();
    }

    private killSpurts(): void {
        let _loc2_: Spurtz | null = null;
        let _loc1_: number = this._spurts.length - 1;
        while (_loc1_ >= 0) {
    ...
    private setAngleToTarget(): void {
        const _loc1_: number = Math.atan2(this.y + Math.abs(this._top) - this._target.y, this.x - this._target.x);
    ...
    private rotateBarrelTowardsTarget(): void {
        const _loc1_: number = this._angleToTarget - this._barrelRotation > 0 ? 1 : -1;
    ...
    private shoot(): void {
        ...
        const _loc1_: number = 0.5 + 0.5 / this.maxHealth * this.health;
        let _loc2_: number = 1;
        const _loc3_: number = MathUtils.getDistanceBetweenTwoPoints(this._position, new Point(this._target.x, this._target.y));
        const _loc4_: number = (this._barrelRotation + 180) * (Math.PI / 180);
        let _loc5_: Point = new Point(this.x + Math.cos(_loc4_) * _loc3_, this.y + Math.sin(_loc4_) * _loc3_);
    ...
    private shootJar(): void {
        ...
        const _loc3_: number = this.damage * 0.25 * _loc1_ * _loc2_;
        this._jarHealth.Add(-Math.floor(_loc3_));
        ATTACK.Damage(this._mc.x, this._mc.y + this._top, _loc3_);
    }

    private scaleDisplayObjectRandomly(param1: DisplayObject): void {
        const _loc2_: number = Math.random() * 0.6 + 0.4;
    ...
    protected collidedWithTarget(param1: Event): void {
        ...
        const _loc3_: number = Math.atan2(_loc2_._startPoint.x - _loc2_._targetPoint.x, _loc2_._startPoint.y - _loc2_._targetPoint.y);
        const _loc4_: number = _loc3_ * (180 / Math.PI);
    ...
    private dealAoEDamage(param1: FIREBALL): void {
        const _loc2_: number = this._projectile._graphic.width + this._projectile._graphic.height;
        const _loc3_: Point = new Point(param1._tmpX, this._projectile._tmpY);
        const _loc4_: any[] = Targeting.getCreepsInRange(_loc2_, _loc3_, Targeting.getOldStyleTargets(0));
        if (_loc4_.length > 0) {
    ...
    private spawnSpurtzAt(param1: number, param2: number, param3: number, param4: number): void {
        const _loc5_: Spurtz = CREATURES.Spawn("IC1", MAP._BUILDINGTOPS, "defend", new Point(param1, param2), param3) as Spurtz;
