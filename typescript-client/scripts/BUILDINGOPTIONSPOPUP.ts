...
export class BUILDINGOPTIONSPOPUP extends BUILDINGOPTIONSPOPUP_CLIP {
    ...
    private Render(param1: string): string {
        ...
        if (param1 == "fortify") {
            ...
            let nextFortifyLevel: number = this._building._fortification.Get() + 1;
            ...
            let imageLevel: number = 1;
            
            if (this._building._lvl.Get() == 0) {
                imageLevel = 1;
            } else {
                let i: number = this._building._lvl.Get();
                if (param1 == "upgrade") {
                    i += 1;
                }
                if (Boolean(imageDataA[i]) && i > this._building._lvl.Get()) {
                    ...
                    let j: number = this._building._lvl.Get();
                    while (j > 0) {
                        if (imageDataA[j]) {
                            imageLevel = j;
                            break;
